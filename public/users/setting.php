...
$select_sth = $dbh->prepare('SELECT id, login_id, password, display_name, self_intro FROM users WHERE id = :id LIMIT 1');
$select_sth->execute([
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	    // 更新
    $update_sth = $dbh->prepare('UPDATE users SET display_name = :display_name, self_intro = :self_intro WHERE id = :id LIMIT 1');
        $update_sth->execute([
...
<body>
  ...
  <form method="post">
    <dl>
      ...
      <dd>
        <textarea name="self_intro" value="<?= htmlspecialchars($login_user["self_intro"]) ?>" placeholder="自己紹介文を入力しましょう"></textarea>
      </dd>
      <hr>
    </dl>
    <div>
